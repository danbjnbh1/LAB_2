...
In this lab, you should perform task 0 before attending the lab session. This lab may be done in pairs (as a rule, members of a pair must be in the same lab group, and should inform the TA(s) about being asuch a pair).
...
First, download the files LineParser.c and LineParser.h. These files contain some useful parsing and string management functions that will simplify your code substantially. Make sure you appropriately refer to LineParser.c in your makefile. You can find a detailed explanation in the reading material for lab 2.
...
All the above constitutes task 0, which you should do and fully understand before attending the lab
...
Invoke waitpid when you're required, and only when you're required. For example: "cat myshell.c &" will not wait for the cat process to end (cat in this case runs in the background), but "cat myshell.c" will (cat runs in the foreground). Additional examples below.

```
myshell> sleep 5
```
The shell freezes for 5 seconds — because it should wait for the child process which is sleeping to wake up and terminate.

```
myshell> sleep 5 &
```
This time the sleep program runs in the background, and the shell can continue receiving commands.
...
```
This creates the file test.txt containing the "text" hello.

```
myshell> cat < input.txt
```
Opens the file input.txt as input and prints its content to the screen.

```
myshell> cat < input.txt > output.txt
```
Reads from input.txt and writes the result into output.txt.
...
- **zzzz <process id>** - Send a SIGSTOP signal to a process to make it "sleep".
- **kuku <process id>** - Wake up a sleeping (stopped) process (SIGCONT).
- **blast <process id>** - Terminate a running/sleeping process (SIGINT).
...
**Your task:** Implement a simple program called mypipe (a program separate from your shell), which creates a child process that sends as a message the value of argv[1] to its parent process. The parent then prints the incoming message and terminates. Use the pipe system call (see man) to create the pipe. Example:

```
myshell> ./mypipe "Hello? Anyone out there?! It's dark in here!"
```
The child process writes the message into the pipe, the parent reads it, and prints it to the screen
...
1. Create a zip file with the relevant files (only) (named either [student-id-num].zip [student1-id-num_student2-id-num.zip] in case of pair submission).
